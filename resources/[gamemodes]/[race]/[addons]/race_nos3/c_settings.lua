...
addCommandHandler("reset_nosgauge", resetSettings)
...
--[[
function consoleSetNosGaugeScale(commandName, number)
	if tonumber(number) == nil then
		outputGuiPopup("Syntax: /scale_nosgauge [number]")
		return
	end

	g_settings.GaugeScale = number

	if saveSettingsToFile() then
		outputGuiPopup("Nos gauge scale is now " .. number)
	end
end
addCommandHandler("scale_nosgauge", consoleSetNosGaugeScale)
]]
