...
Bigdar.enabled = false		-- Manual override
...
function Bigdar.toggle()
	...
	outputChatBox( '#ADADADinfo: Traffic Sensor is now ' .. (Bigdar.enabled and 'on' or 'off') .. "." )
end
...
function onHotKey(player, arg1)
	if arg1 == 'always' then
		carhideBypass = not carhideBypass
		if carhideBypass == true then
			bigdarTrue()
			outputChatBox("#ADADADinfo: Always-on traffic sensor on.", 255, 255, 255, true)
		else
			outputChatBox("#ADADADinfo: Always-on traffic sensor off.", 255, 255, 255, true)
		end
	elseif arg1 == '' then
		Bigdar.toggle()
	end
end
addCommandHandler ( "trafficsensor", onHotKey )
bindkey("F4", onHotKey)


function bigdarTrue()
	if carhideBypass == false then
		Bigdar.enabled = true
	end
end

function bigdarFalse()
	if carhideBypass == false then
