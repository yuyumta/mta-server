addCommandHandler("mapname",
function ( playerSource, commandName, name )
	local n = call(getResourceFromName("mapmanager"), "getRunningGamemodeMap")
	outputChatBox(tostring(getResourceName(n)))
	call(getResourceFromName("race"), "startRandomMap")
end
)
...
function ( playerSource, commandName)
    local name = getResourceName(call(getResourceFromName("mapmanager"), "getRunningGamemodeMap"))
