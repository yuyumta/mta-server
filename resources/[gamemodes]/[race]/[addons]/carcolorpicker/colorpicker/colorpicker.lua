...
colorPicker = {
	...
	constructor = function( info )
		...
		if onlystart then
		colorPicker.value = { rstart, gstart, bstart, 255 }
		onlystart = false
		else
		...
		colorPicker.GUI.noteLabel = guiCreateLabel(info.selectWindow.noteX, info.selectWindow.noteY,
		                                       190, 15, "Click outside the window or Press "..g_colorkey.." to close.", false, colorPicker.GUI.selectWindow)
		...
		checkColor4 = guiCreateCheckBox(info.selectWindow.ra4X, info.selectWindow.ra4Y,
												75, 15, "Random", randkeep, false, colorPicker.GUI.selectWindow)
...
--This is where the color updates every time you click the palette or presets!
	...
	closeSelect = function()
		...
		--outputChatBox(tostring(colorPicker.value[1])..", "..tostring(colorPicker.value[2])..", "..tostring(colorPicker.value[3]))
		showCursor(false)
	end,
...
function startvalue ( r1, g1, b1, g_randlast)
    --outputChatBox("start")
    ...
    onlystart = true
    --outputChatBox("testing works." .. r1 , true )
    rstart = r1
    gstart = g1
    bstart = b1
    colorPicker.constructor(colorPicker.default)
end
addEvent( "startvalue", true )
addEventHandler( "startvalue", getRootElement(), startvalue )
